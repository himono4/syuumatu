tyrano.plugin.kag.studio = {
  ...
  trigger(cmd, arg) {
    if ("ping" == cmd) this.send("asynchronous-reply", JSON.stringify(arg));
    else if ("variable-add" == cmd) {
    ...
    } else if ("status-clear-save-data" == cmd) localStorage.clear();
    else if ("status-load-save" == cmd) {
    ...
    } else if ("material-preview-position" == cmd) {
      let data = JSON.parse(arg),
        file = data.file,
        category = data.category;
      this.insertElement(category, file);
    } else if ("variable-add-all" == cmd) {
