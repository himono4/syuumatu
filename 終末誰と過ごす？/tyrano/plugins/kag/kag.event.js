tyrano.plugin.kag.event = {
  ...
  addRestoreData: function (j_obj, tag, pm) {
    ...
    let restore_data;
    restore_data =
      "string" == typeof restore_data_str ? JSON.parse(restore_data_str) : [];
    restore_data.push({ tag: tag, pm: pm });
  ...
  removeRestoreData: function (j_obj, tag) {
    ...
    const filterd_data = JSON.parse(restore_data_str).filter(
      (item) => item.tag !== tag
    );
    filterd_data.length > 0
