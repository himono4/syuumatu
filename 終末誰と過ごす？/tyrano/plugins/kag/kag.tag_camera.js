tyrano.plugin.kag.tag.camera = {
  ...
  start: function (pm) {
    ...
    if (
      ...
      const from_info = { x: "0", y: "0", scale: "1", rotate: "0" };
      "" !== pm.from_x && (from_info.x = pm.from_x);
      "" !== pm.from_y && (from_info.y = pm.from_y);
      "" !== pm.from_zoom && (from_info.scale = pm.from_zoom);
      "" !== pm.from_rotate && (from_info.rotate = pm.from_rotate);
      this.kag.stat.current_camera[pm.layer] = {
        x: -1 * parseInt(from_info.x) + "px",
        y: 1 * parseInt(from_info.y) + "px",
