tyrano.plugin.kag.tag.playbgm = {
  ...
  start: function (pm) {
    const should_next_order = "false" === pm.stop;
    if ("bgm" == pm.target && !1 === this.kag.stat.play_bgm) {
      should_next_order && this.kag.ftag.nextOrder();
      return;
    }
    if ("se" == pm.target && !1 === this.kag.stat.play_se) {
      should_next_order && this.kag.ftag.nextOrder();
      return;
    }
    let can_ignore_in_no_ready = !1;
    if (!should_next_order) {
      can_ignore_in_no_ready = !0;
      "false" === pm.can_ignore && (can_ignore_in_no_ready = !1);
    }
    "pc" === $.userenv()
      ? this.kag.tmp.ready_audio
        ? this.play(pm)
        : can_ignore_in_no_ready || this.waitClick(pm)
      : this.kag.stat.is_skip && "se" === pm.target
      ? should_next_order && this.kag.ftag.nextOrder()
      : this.kag.tmp.ready_audio
      ? this.play(pm)
      : can_ignore_in_no_ready || this.waitClick(pm);
  ...
  parseMilliSeconds: function (_str) {
    const str = $.trim(_str),
      colon_count = (str.match(/:/g) || []).length;
    if (0 === colon_count) return parseInt(str);
    let hours_str = "0",
      minutes_str = "0",
      seconds_str = "0",
      milli_seconds_str = "0";
    ...
    return (
      1e3 * (parseInt(hours_str) || 0) * 60 * 60 +
      1e3 * (parseInt(minutes_str) || 0) * 60 +
      1e3 * (parseInt(seconds_str) || 0) +
      (parseInt(milli_seconds_str) || 0)
    );
  },
  play: async function (pm) {
    const is_se = "se" === pm.target,
      buf = pm.buf,
      is_voice = !!this.kag.stat.map_vo.vobuf[buf],
      is_loop = "true" === pm.loop;
    let is_fadein = "true" === pm.fadein;
    (this.kag.stat.is_skip || 0 === parseInt(pm.time)) && (is_fadein = !1);
    const should_next_order = "false" === pm.stop;
    should_next_order && this.kag.weaklyStop();
    let sound_type = is_se ? "sound" : "bgm",
      timeout_after_voice_in_automode = 500;
    void 0 !== this.kag.stat.voconfig_waittime &&
      (timeout_after_voice_in_automode = parseInt(
        this.kag.stat.voconfig_waittime
      ));
    const next = () => {
      if (should_next_order) {
    ...
    switch (sound_type) {
      ...
      case "sound":
        ...
        is_voice && (this.kag.tmp.is_vo_play = !0);
    }
    if ("" != pm.base64) {
      storage =
        "data:audio/" +
        pm.base64 +
        ";base64," +
        (await $.loadTextSync(storage));
      this.kag.stat.current_bgm_base64 = pm.base64;
    } else this.kag.stat.current_bgm_base64 = "";
    var howl_opt = {
      preload: !1,
      loop: is_loop,
      src: storage,
    ...
    let sprite_name, preloaded_audio_del;
    if (pm.sprite_time) {
      const array_sprite = pm.sprite_time.split("-"),
        sprite_from = this.parseMilliSeconds(array_sprite[0]),
        duration = this.parseMilliSeconds(array_sprite[1]) - sprite_from,
        sprite = {};
    ...
    audio_obj.once("load", () => {
      ...
      "" != pm.seek && audio_obj.seek(parseFloat(pm.seek));
      "true" != pm.pause ? audio_obj.play(sprite_name) : next();
    });
    audio_obj.once("play", () => {
      is_fadein && audio_obj.fade(0, howl_volume, parseInt(pm.time));
      if (is_voice) this.analyzeAudioForLipSync(audio_obj, pm.chara_name);
      else if (pm.chara) this.analyzeAudioForLipSync(audio_obj, pm.chara);
      else if (is_se) {
        ...
        if (this.kag.stat.lipsync_buf_chara[buf]) {
          ...
          if (pm.chara_name) {
            ...
            cpm &&
              cpm.lipsync_bufs &&
              cpm.lipsync_bufs.includes(_buf) &&
              this.analyzeAudioForLipSync(audio_obj, pm.chara_name);
          }
        }
      }
    ...
    is_loop ||
      audio_obj.once("end", (e) => {
        switch (sound_type) {
          case "bgm":
            this.kag.tmp.is_bgm_play = !1;
            if (1 == this.kag.tmp.is_bgm_play_wait) {
              this.kag.tmp.is_bgm_play_wait = !1;
              this.kag.ftag.nextOrder();
            }
            break;
          case "sound":
            this.kag.tmp.is_se_play = !1;
            is_voice && (this.kag.tmp.is_vo_play = !1);
            if (1 == this.kag.tmp.is_se_play_wait) {
              let is_sound_playing = !1;
              for (const key in this.kag.tmp.map_se) {
                const howl = this.kag.tmp.map_se[key];
                if (!howl._loop && howl.playing()) {
                  is_sound_playing = !0;
                  break;
                }
              }
              if (!is_sound_playing) {
                this.kag.tmp.is_se_play_wait = !1;
                this.kag.ftag.nextOrder();
              }
            } else if (1 == this.kag.tmp.is_vo_play_wait && is_voice) {
              this.kag.tmp.is_vo_play_wait = !1;
              $.setTimeout(() => {
                this.kag.ftag.nextOrder();
  ...
  analyzeAudioForLipSync(howl, name) {
    ...
    const audio_context = Howler.ctx,
      sound_node = howl._sounds[0]._node,
    ...
    const buffer_length = analyser.frequencyBinCount,
      ...
      analyze = () => {
        ...
        const volume = (((max - 128) / 127) * 100) | 0;
        this.kag.chara.updateLipSyncWithVoice(
          volume,
          target_parts,
          elapsed_time
        );
        max <= 128 ? (silent_time += elapsed_time) : (silent_time = 0);
        if (silent_time >= 1e4) resetFrameOpacity();
        else {
          last_timestamp = timestamp;
          animation_id = setTimeout(analyze, 1e3 / 30);
        }
      };
...
tyrano.plugin.kag.tag.stopbgm = {
  ...
  start: function (pm) {
    const should_next_order = "false" === pm.stop;
    let target_map = null,
      is_fadeout = "true" === pm.fadeout;
    (this.kag.stat.is_skip || 0 === parseInt(pm.time)) && (is_fadeout = !1);
    if ("bgm" == pm.target) {
    ...
    for (const key in target_map) {
      if (!(String(key) === String(pm.buf) || "true" === pm.buf_all)) continue;
      ...
      if (audio_obj)
        if (is_fadeout && audio_obj.playing()) {
          const bind_fade_complete_listener = () => {
            audio_obj.once("fade", () => {
              if (0 === audio_obj.volume()) {
                audio_obj.stop();
                audio_obj.unload();
              } else bind_fade_complete_listener();
            });
          };
...
tyrano.plugin.kag.tag.xchgbgm = {
  ...
  start: function (pm) {
    if (this.kag.stat.is_skip || 0 === parseInt(pm.time)) {
      pm.time = "0";
      pm.fadein = "false";
      this.kag.ftag.startTag("playbgm", pm);
      return;
...
tyrano.plugin.kag.tag.bgmopt = {
  ...
  start: function (pm) {
    let tag_volume, config_volume;
    if (pm.buf) {
      "" !== pm.volume && (this.kag.stat.map_bgm_volume[pm.buf] = pm.volume);
    ...
    if ("true" == pm.effect && 0 == this.kag.define.FLAG_APRI) {
      const options = { config: config_volume, tag: tag_volume, time: pm.time };
      if (pm.buf) {
        const audio_obj = target_map[pm.buf];
        audio_obj && this.kag.changeHowlVolume(audio_obj, options);
      } else
        for (const key in target_map) {
          const audio_obj = target_map[key];
          audio_obj && this.kag.changeHowlVolume(audio_obj, options);
    ...
    pm.samebgm_restart &&
      (this.kag.stat.bgmopt_samebgm_restart = "true" === pm.samebgm_restart);
    void 0 !== pm.volume && "" !== pm.volume
...
tyrano.plugin.kag.tag.seopt = {
  pm: { volume: "", effect: "true", buf: "", tag_volume: "", next: "true" },
  start: function (pm) {
    let tag_volume, config_volume;
    if (pm.buf) {
      "" !== pm.volume && (this.kag.stat.map_se_volume[pm.buf] = pm.volume);
...
tyrano.plugin.kag.tag.changevol = {
  pm: { target: "bgm", volume: "", buf: "", time: "", next: "true" },
  obtainTargets: function (target, buf) {
    const target_map =
        "bgm" === target ? this.kag.tmp.map_bgm : this.kag.tmp.map_se,
      target_dict = {};
    if (buf) {
      const audio_obj = target_map[buf];
      audio_obj && (target_dict[buf] = audio_obj);
    } else
      for (const key in target_map) {
        const audio_obj = target_map[key];
        audio_obj && (target_dict[key] = audio_obj);
      }
    return target_dict;
  },
  start: function (pm) {
    ...
    const tag_volume = $.parseVolume(pm.volume),
      is_bgm = "bgm" === pm.target;
    is_bgm && (this.kag.stat.current_bgm_vol = pm.volume);
    const volume_map = is_bgm
        ? this.kag.stat.map_bgm_volume
        : this.kag.stat.map_se_volume,
      default_volume = is_bgm
        ? this.kag.config.defaultBgmVolume
        : this.kag.config.defaultSeVolume,
      target_dict = this.obtainTargets(pm.target, pm.buf);
...
tyrano.plugin.kag.tag.pausebgm = {
  pm: { target: "bgm", buf: "", next: "true" },
  start: function (pm) {
    const target_dict = this.kag
      .getTag("changevol")
      .obtainTargets(pm.target, pm.buf);
    for (const buf in target_dict) {
      const audio_obj = target_dict[buf];
      audio_obj.pause();
  ...
  start: function (pm) {
    ...
    if ("" != pm.name) {
      ...
      vochara = this.kag.stat.map_vo.vochara[pm.name]
        ...
        : { vostorage: "", buf: pm.sebuf || "0", number: 0 };
      "" !== pm.sebuf && (vochara.buf = pm.sebuf);
    ...
    pm.waittime && (this.kag.stat.voconfig_waittime = parseInt(pm.waittime));
    pm.preload && (this.kag.stat.voconfig_preload = "true" === pm.preload);
    this.kag.ftag.nextOrder();
  },
};
...
tyrano.plugin.kag.tag.speak_on = {
  vital: [],
  pm: { volume: "100", pitch: "100", rate: "100", cancel: "false" },
  start: function (pm) {
    ...
    pm.volume && (this.kag.tmp.speak_on_volume = parseInt(pm.volume) / 100);
    pm.pitch && (this.kag.tmp.speak_on_pitch = parseInt(pm.pitch) / 100);
    pm.rate && (this.kag.tmp.speak_on_rate = parseInt(pm.rate) / 100);
    pm.cancel && (this.kag.tmp.speak_on_cancel = "true" === pm.cancel);
