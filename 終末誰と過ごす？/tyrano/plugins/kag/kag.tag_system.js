...
tyrano.plugin.kag.tag.clearstack = {
	...
	start: function(pm) {
		"" == pm.stack ? this.kag.stat.stack = {
			...
			call: [],
			macro: []
		} : "anim" === pm.stack ? this.kag.tmp.num_anim = 0 : this.kag.stat.stack[
			pm.stack] = [];
		this.kag.ftag.nextOrder()
	}
};
...
tyrano.plugin.kag.tag.return = {
	start: function() {
		...
		if (pm.caller && pm.caller.storage && ("make.ks" == pm.caller.storage || pm
				.caller.storage == this.kag.stat.resizecall.storage) && 1 == this.kag.tmp
			.loading_make_ref) {
...
tyrano.plugin.kag.tag.rollback = {
	...
	start: function(pm) {
		if (0 == this.kag.menu.doRollback(pm.checkpoint, pm.variable_over, pm.bgm_over)) {
...
tyrano.plugin.kag.tag.clear_checkpoint = {
	...
	start: function(pm) {
		"" == pm.name ? this.kag.stat.checkpoint = {} : delete this.kag.stat.checkpoint[
			pm.name];
		this.kag.ftag.nextOrder()
	}
};
...
tyrano.plugin.kag.tag.cursor = {
	...
	start: function(pm) {
		...
		if (pm.click_effect) {
			...
			"function" == typeof this.kag.tmp.show_effect_callback && document.body.removeEventListener(
				event_type, this.kag.tmp.show_effect_callback, {
					capture: !0
				});
			this.kag.stat.click_effect.is_enabled = "true" === pm.click_effect;
			if (this.kag.stat.click_effect.is_enabled) {
				"function" != typeof this.kag.tmp.show_effect_callback && (this.kag.tmp.show_effect_callback =
					e => {
						this.showEffect(e)
					});
	...
	restore: function() {
		...
		this.kag.ftag.startTag("cursor", {
			click_effect: String(this.kag.stat.click_effect && this.kag.stat.click_effect
				.is_enabled),
			next: "false"
		})
	},
...
tyrano.plugin.kag.tag.loadcss = {
	...
	start: function(pm) {
		var file = pm.file,
			style = '<link class="_tyrano_cssload_tag" rel="stylesheet" href="' + file +
...
tyrano.plugin.kag.tag.dialog_config = {
	...
	init() {
		if (!this.__initialized) {
			...
			this.j_cancel = this.j_box.find(".remodal-cancel").attr("id",
				"remodal-cancel");
			this.j_ok = this.j_box.find(".remodal-confirm").attr("id",
				"remodal-confirm");
	...
	start: function(pm) {
		...
		pm.btnfontbold && this.j_button.setStyle("font-weight", $.convertFontWeight(
			pm.btnfontbold));
		...
		if (pm.boxradius) {
			...
			this.j_boxbase.setStyle("border-radius", $.convertLength(pm.boxradius +
				"px"))
		}
		pm.boxcolor && this.j_boxbase.setStyle("background-color", $.convertColor(
			pm.boxcolor));
		pm.boximg && this.j_boxbase.setStyle("background-image", $.convertBackgroundImage(
			pm.boximg, "image"));
		pm.boximgpos && this.j_boxbase.setStyle("background-position", $.convertBackgroundPosition(
			pm.boximgpos));
		pm.boximgsize && this.j_boxbase.setStyle("background-size", $.convertLength(
			pm.boximgsize));
		...
		pm.boxwidth && this.j_box.setStyle("width", $.convertLength(pm.boxwidth)).setStyle(
			"max-width", $.convertLength(pm.boxwidth));
		pm.boxheight && this.j_box.setStyle("height", $.convertLength(pm.boxheight))
		...
		pm.bgimg && this.j_image.setStyle("background-image", $.convertBackgroundImage(
			pm.bgimg, "image"));
		pm.bgimgpos && this.j_image.setStyle("background-position", $.convertBackgroundPosition(
			pm.bgimgpos));
		pm.bgimgsize && this.j_image.setStyle("background-size", $.convertLength(pm
			.bgimgsize));
		...
		pm.opentime && (this.kag.tmp.remodal_opening_effect_time = pm.opentime +
			"ms");
		pm.closetime && (this.kag.tmp.remodal_closing_effect_time = pm.closetime +
			"ms");
	...
	changeButton(pm, is_ok) {
		...
		pm.img && (this.css_map[`#${id}`]["background-image"] = $.convertBackgroundImage(
			pm.img, "image"));
		if (pm.activeimg) {
			this.css_map[`#${id}:active`]["background-image"] = $.convertBackgroundImage(
				pm.activeimg, "image");
			this.css_map[`#${id}.active`]["background-image"] = $.convertBackgroundImage(
				pm.activeimg, "image")
		}
		if (pm.enterimg) {
			this.css_map[`#${id}:hover`]["background-image"] = $.convertBackgroundImage(
				pm.enterimg, "image");
			this.css_map[`#${id}.hover`]["background-image"] = $.convertBackgroundImage(
				pm.enterimg, "image")
		}
		pm.clickimg && (this.css_map[`#${id}.clicked`]["background-image"] = $.convertBackgroundImage(
			pm.clickimg, "image"));
		this.updateCSS();
	...
	createButton(pm) {
		...
		j_img.on("mouseenter", (() => {
			pm.enterimg && (pm.activeimg && 0 !== j_img.filter(":active").length ||
				j_img.attr("src", pm.enterimg));
			pm.enterse && this.kag.playSound(pm.enterse)
		}));
		j_img.on("mouseleave", (() => {
			pm.activeimg ? clicked || 0 !== j_img.filter(":active").length || j_img.attr(
				"src", pm.img) : clicked || j_img.attr("src", pm.img);
			pm.leavese && this.kag.playSound(pm.leavese)
		}));
		j_img.on("mousedown", (() => {
		...
		j_img.on("activeoff", (() => {
			j_img.filter(":hover").length > 0 ? clicked = !0 : pm.activeimg && j_img
				.attr("src", pm.img)
		}));
		j_img.on("click", (() => {
			clicked = !0;
