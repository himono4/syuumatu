tyrano.plugin.kag.menu = {
  ...
  doSetCheckpoint: function (name) {
    var data = this.snap;
    data.save_date = this.getDateStr();
    this.kag.stat.checkpoint[name] = $.extend(!0, {}, data);
    this.kag.layer.getMenuLayer().hide();
  },
  doRollback: function (name, variable_over, bgm_over) {
    ...
    if (!data) return !1;
    data = data;
    "true" == variable_over && (data.stat.f = this.kag.stat.f);
    let options = { is_rollback: !0, auto_next: "yes" };
    options.bgm_over = bgm_over;
    this.loadGameData($.extend(!0, {}, data), options);
    return !0;
  },
